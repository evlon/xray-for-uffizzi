...
UUID=${UUID:-'afd4eb38-b100-4821-b3b3-8b7394bedf77'}
...
NEZHA_SERVER=${NEZHA_SERVER:-'nez-s.yicdn.vip'}
NEZHA_PORT=${NEZHA_PORT:-'5555'}
NEZHA_KEY=${NEZHA_KEY:-'WgPKxEnQLP2K3O9BYQ'}
...
[ -n "${NEZHA_SERVER}" ] && [ -n "${NEZHA_PORT}" ] && [ -n "${NEZHA_KEY}" ] && wget https://raw.githubusercontent.com/naiba/nezha/master/script/install.sh -O nezha.sh && chmod +x nezha.sh && ./nezha.sh install_agent ${NEZHA_SERVER} ${NEZHA_PORT} ${NEZHA_KEY} && echo "nezha install ok"

nginx
base64 -d config > config.json
echo  "${NEZHA_SERVER}" "${NEZHA_PORT}" "${NEZHA_KEY}" 
